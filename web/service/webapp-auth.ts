...
import { getPublic, getWebChat, post, postPublic } from './base'
...
export async function chatAppLoginStatus(shareCode: string) {
  const { logged_in, app_logged_in } = await getWebChat<isWebAppLogin>(`/login/status?app_code=${shareCode}`)
  return {
    userLoggedIn: logged_in,
    appLoggedIn: app_logged_in,
  }
...
export async function chatAppLogout() {
  clearWebAppAccessToken()
  clearWebAppPassport('chat-app')
  await post('/logout')
}
