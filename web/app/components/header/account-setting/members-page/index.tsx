              ...
              accounts.map(account => (
                <div key={account.id} className='flex border-b border-divider-subtle'>
                  ...
                  <div className='flex w-[96px] shrink-0 items-center'>
                    {(() => {
                      if (isCurrentWorkspaceOwner && account.role !== 'owner')
                        return <Operation member={account} operatorRole={currentWorkspace.role} onOperate={mutate} />
                      else if (isCurrentWorkspaceManager && account.role !== 'owner' && userProfile.email !== account.email)
                        return <Operation member={account} operatorRole={currentWorkspace.role} onOperate={mutate} />
                      else
                        return <div className='system-sm-regular px-3 text-text-secondary'>{RoleMap[account.role] || RoleMap.normal}</div>
                    })()}
                  </div>
                </div>
              ))
