...
@app.route("/upload_files/<file_folder>/<filename>", methods=["GET"])
def view_pdf(file_folder, filename):
    """
    显示指定的 PDF 文件。
    """
    ...
    try:
        ...
        DOCUMENT_LOCATION = os.path.normpath(
            os.path.join(
                os.getcwd(), 
                dify_config.STORAGE_LOCAL_PATH, 
                "upload_files/{}/{}".format(file_folder, filename)
            )
        )
