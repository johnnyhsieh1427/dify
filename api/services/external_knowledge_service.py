...
class ExternalDatasetService:
    ...
    def get_external_knowledge_apis(page, per_page, tenant_id, search=None) -> tuple[list[ExternalKnowledgeApis], int]:
        query = db.session.query(ExternalKnowledgeApis).filter(ExternalKnowledgeApis.tenant_id == tenant_id).order_by(
    ...
    def get_external_knowledge_api(external_knowledge_api_id: str) -> ExternalKnowledgeApis:
        external_knowledge_api: Optional[ExternalKnowledgeApis] = db.session.query(ExternalKnowledgeApis).filter_by(
    ...
    def delete_external_knowledge_api(tenant_id: str, external_knowledge_api_id: str):
        external_knowledge_api = db.session.query(ExternalKnowledgeApis).filter_by(
            id=external_knowledge_api_id, tenant_id=tenant_id
        ).first()
        if external_knowledge_api is None:
            raise ValueError("api template not found")
    ...
    def fetch_external_knowledge_retrieval(
        ...
        external_knowledge_binding = db.session.query(ExternalKnowledgeBindings).filter_by(
