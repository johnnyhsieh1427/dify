...
class DatasetService:
    ...
    def get_datasets(page, per_page, tenant_id=None, user=None, search=None, tag_ids=None, include_all=False):
        ...
        if user:
            ...
            dataset_permission_all_team_members = (
                db.session.query(Dataset).filter_by(permission=DatasetPermissionEnum.ALL_TEAM).all()
            )
            permitted_dataset_ids = {
                dp.dataset_id for dp in dataset_permission
            } if dataset_permission else set()
            
            permitted_dataset_ids.update(
                ds.id for ds in dataset_permission_all_team_members
            ) if dataset_permission_all_team_members else None
