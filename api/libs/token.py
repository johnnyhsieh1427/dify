...
def extract_chat_app_passport(request: Request) -> str | None:
    def _try_extract_passport_token_from_cookie(request: Request) -> str | None:
        return request.cookies.get(_real_cookie_name(COOKIE_NAME_PASSPORT + "-" + "chat-app"))

    ret = _try_extract_passport_token_from_cookie(request)
    return ret
...
def set_chat_app_token_to_cookie(request: Request, response: Response, token: str):
    response.set_cookie(
        _real_cookie_name(COOKIE_NAME_PASSPORT + "-" + "chat-app"),
        value=token,
        httponly=True,
        secure=is_secure(),
        samesite="Lax",
