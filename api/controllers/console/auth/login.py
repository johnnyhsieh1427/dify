...
class LoginApi(Resource):
    ...
    def post(self):
        ...
        is_login_error_rate_limit = AccountService.is_login_error_rate_limit(user_email)
        if is_login_error_rate_limit:
            raise EmailPasswordLoginLimitError()

        invitation = args["invite_token"]
        if invitation:
            invitation = RegisterService.get_invitation_if_token_valid(None, user_email, invitation)
        ...
        try:
            if invitation:
                ...
                account = AccountService.authenticate(user_email, args["password"], args["invite_token"])
            else:
                account = AccountService.authenticate(user_email, args["password"])
        except services.errors.account.AccountLoginError:
            raise AccountBannedError()
        except services.errors.account.AccountPasswordError:
            AccountService.add_login_error_rate_limit(user_email)
        ...
        set_chat_app_token_to_cookie(request, response, token_pair.chat_app_token)
    ...
    def post(self):
        ...
        if account is None:
            if FeatureService.get_system_features().is_allow_register:
                token = AccountService.send_email_code_login_email(email=user_email, language=language)
            else:
...
class RefreshTokenApi(Resource):
    def post(self):
        ...
        try:
            ...
            set_chat_app_token_to_cookie(request, response, new_token_pair.chat_app_token)
